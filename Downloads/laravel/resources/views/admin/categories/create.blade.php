...
@section('header', __('admin.add_category'))

@section('content')
    <div class="max-w-2xl">
        <form action="{{ route('admin.categories.store') }}" method="POST" class="bg-white rounded-xl shadow-sm p-6">
            @csrf
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" name="name_kk" value="{{ old('name_kk') }}" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <textarea name="description_kk" rows="2" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">{{ old('description_kk') }}</textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('admin.description_ru') }}</label>
                        <textarea name="description_ru" rows="2" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">{{ old('description_ru') }}</textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('admin.description_en') }}</label>
                        <textarea name="description_en" rows="2" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">{{ old('description_en') }}</textarea>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="color" name="color" value="{{ old('color', '#3B82F6') }}" required
